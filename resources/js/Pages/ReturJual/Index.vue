...
const canViewReturJual = computed(() => props.permissions.retur_jual_view);
const canCreateReturJual = computed(() => props.permissions.retur_jual_create);
const canEditReturJual = computed(() => props.permissions.retur_jual_edit);
const canDeleteReturJual = computed(() => props.permissions.retur_jual_delete);
const canApproveReturJual = computed(
    () => props.permissions.retur_jual_approve,
);
...
const onSalesSelect = async (sales) => {
    try {
        const response = await fetch(
            `http://127.0.0.1:8000/api/retur-jual/sales/${sales.id}`,
        );
        if (!response.ok) {
        ...
        form.details = poDetails.map((detail) => ({
            ...detail,
            qty_jual: detail.qty,
            nama_satuan_jual: detail.nama_satuan,
            qty_retur: 0,
...
const columns = [
    {
        accessorKey: "kode_retur_jual",
        header: () => h("div", { class: "text-left" }, "Kode Retur Jual"),
        cell: ({ row }) => {
            return h(
                "div",
                { class: "text-left font-medium" },
                row.getValue("kode_retur_jual"),
            );
        },
    },
    {
...
const submit = () => {
    const url = form.id ? `/retur-jual/${form.id}` : "/retur-jual";
...
const onEdit = async (id) => {
    if (canEditReturJual.value) {
        showCreate.value = true;
        try {
            const res = await fetch(`/retur-jual/${id}`, {
                method: "GET",
                headers: {
            ...
            form.kode_penjualan = data.data.kode_penjualan || null;
            form.details = [];
...
const onPrint = async (id) => {
    axios({
        url: `http://127.0.0.1:8000/api/retur-jual/print/${id}`,
        method: "GET",
        headers: {
        ...
        if (window.focus) {
            ...
            fileLink.setAttribute("stream", "ReturJual.pdf");
...
<template>
    <Layout>
        <div class="flex items-center">
            <h1 class="text-lg font-semibold md:text-2xl">Retur Jual</h1>
        </div>
        <div v-if="canViewReturJual" class="w-full">
            <div class="flex items-center justify-between py-4">
                <Input
                    :model-value="
                        table.getColumn('kode_retur_jual')?.getFilterValue()
                    "
                    class="max-w-sm"
                    placeholder="Filter kode retur..."
                    @update:model-value="
                        table
                            .getColumn('kode_retur_jual')
            ...
            <Form>
                ...
                >
                    ...
                    <div class="flex flex-row justify-start gap-4">
                        <div>
                            ...
                            <SearchableSelect
                                ...
                                api-endpoint="http://127.0.0.1:8000/api/retur-jual/sales"
                                value-field="kode_penjualan"
                                :display-fields="['kode_penjualan']"
                                search-param="search"
                                :per-page="10"
                                :debounce-time="300"
                                loading-text="Loading sales..."
                                no-results-text="No sales found"
                                load-more-text="Load more sales"
                                @select="onSalesSelect"
                            />
                            <p
                                v-if="form.errors.kode_penjualan"
                                class="mt-1 text-sm text-red-500"
                            >
                                {{ form.errors.kode_penjualan }}
                            </p>
                        </div>
                    </div>
                    <div>
                    ...
                    <DialogHeader class="mt-4">
                        <DialogTitle>Data Detail Retur Jual</DialogTitle>
                        <DialogDescription>
